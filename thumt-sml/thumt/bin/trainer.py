...
#        binary_loss = [ce_loss, sp_loss, coh_loss, clus_loss]
#        generation_loss = [ce_loss, kl_loss, bow_loss, clm_loss]
#        code.interact(local=locals())
        '''
        var_binary = tf.get_collection(tf.GraphKeys.GLOBAL_VARIABLES, 'transformer/encoder/layer_5')
        var_generation = tf.get_collection(tf.GraphKeys.GLOBAL_VARIABLES, 'transformer/decoder/layer_5')
        grads_and_vars = compute_gradients(binary_loss, generation_loss, var_binary, var_generation)
        # Optimization
        '''

        grads_and_vars = opt.compute_gradients(
            all_loss, colocate_gradients_with_ops=True)
